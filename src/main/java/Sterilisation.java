...
public class Sterilisation extends Power {
	...
	private static final String STERILISATION_SOUND_PATH = "sterilisationSound.mp3";
	...
	Sterilisation(Controller controller, int xPos, int yPos) {
	...
	void onTick(ArrayList<Rat> rats, Tile currentTile) {
		...
		if (ticksActive == 1) {
			...
			seaSim.playAudioClip(STERILISATION_SOUND_PATH, STERILISATION_SOUND_VOLUME);
		}
	 ...
	 *
	 * @return All Tiles that Sterilisation can reach in all 4 directions. (radius =
	 *         2)
	 */
	private ArrayList<Tile> findPathTiles() {
		...
		while (counter < 3) {
			tilesToSterilise.add(this.getController().getTileAt(this.xPos, this.yPos + counter));

			tilesToSterilise.add(this.getController().getTileAt(this.xPos, this.yPos - counter));

			tilesToSterilise.add(this.getController().getTileAt(this.xPos + counter, this.yPos));
