...
public class LevelFileReader {
	 ...
	 */
	public static void saveLevel(Controller controller, String levelName) throws IOException {
...
//		}
		...
		for (int i = 0; i < controller.getCounters().length; i++) {
			inventory.append(controller.getCounters()[i]).append(",");
		}
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				if (Objects.requireNonNull(controller.getTileAt(x, y)).getOccupantRats().size() > 0) {
					for (Rat rat : Objects.requireNonNull(controller.getTileAt(x, y)).getOccupantRats()) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				if (Objects.requireNonNull(controller.getTileAt(x, y)).getActivePowers().size() > 0) {
					for (Power power : Objects.requireNonNull(controller.getTileAt(x, y)).getActivePowers()) {
		...
		int timeInSeconds = (int) Math.floor(controller.getCurrentTimeLeft() / milli);
	 ...
	 */
	public static void loadNormalLevelFile(Controller controller, String filename, boolean loadDefaultObjects)
		...
		if (loadDefaultObjects) {
			readObjects(controller, reader, loadDefaultObjects);
		}
	 ...
	 */
	public static void loadSavedLevelFile(Controller controller, String filename) throws FileNotFoundException {
		File levelData = new File(filename + ".txt");
		Scanner reader = new Scanner(levelData);
		levelPath = reader.nextLine();
		loadNormalLevelFile(controller, levelPath, false);
		inProgInv = new int[8];
		if (reader.hasNextLine()) {
			...
			for (int i = 0; i < inProgInvString.length; i++) {
				...
				controller.addPowersFromSave(inProgInv);
			}
			readObjects(controller, reader, false);
		}
	}
	...
	/**
	 * Deletes all in progress files (from profile directory) which are connected to
	 * chosen level.
	 *
	 ...
	 */
	private static void readObjects(Controller controller, Scanner reader, boolean readRats) {
		while (reader.hasNextLine()) {
			...
			case "f": {
				...
				ChildRat newRat = new ChildRat(controller, speed, direction, gasTimer, xPos, yPos, isFertile, age,
						Rat.Sex.FEMALE);
			...
			case "F": {
				...
				AdultFemale newRat = new AdultFemale(controller, speed, direction, gasTimer, xPos, yPos, isFertile,
			...
			case "M": {
				...
				boolean isFertile;
				isFertile = currentItem[6].equals("1");
				AdultMale newRat = new AdultMale(controller, speed, direction, gasTimer, xPos, yPos, isFertile);
				RAT_ARRAY_LIST.add(newRat);
				tileMap[xPos][yPos].addOccupantRat(newRat);
				break;
			}
			...
			case "I": {
				...
				int pregnancyTimer = Integer.parseInt(currentItem[7]);
				int ratFetusCount = Integer.parseInt(currentItem[8]);
				AdultIntersex newRat = new AdultIntersex(controller, speed, direction, gasTimer, xPos, yPos, isFertile,
						pregnancyTimer, ratFetusCount);
				RAT_ARRAY_LIST.add(newRat);
				tileMap[xPos][yPos].addOccupantRat(newRat);
				break;
			...
			case "D": {
				...
				DeathRat newRat = new DeathRat(controller, speed, direction, gasTimer, xPos, yPos, killCounter);
				RAT_ARRAY_LIST.add(newRat);
				tileMap[xPos][yPos].addOccupantRat(newRat);
				break;
			}
			...
			case "B": {
				...
				Bomb newBomb = new Bomb(controller, xPos, yPos);
			...
			case "G": {
				...
				Gas newGas = new Gas(controller, xPos, yPos, isOriginal);
			...
			case "S": {
				...
				int yPos = Integer.parseInt(currentItem[2]);
				int ticksActive = Integer.parseInt(currentItem[3]);
				Sterilisation newSterilisation = new Sterilisation(controller, xPos, yPos);
			...
			case "P": {
				int xPos = Integer.parseInt(currentItem[1]);
				int yPos = Integer.parseInt(currentItem[2]);
				Poison newPoison = new Poison(controller, xPos, yPos);
			...
			case "T": {
				int xPos = Integer.parseInt(currentItem[1]);
				int yPos = Integer.parseInt(currentItem[2]);
				MaleSwapper newMaleSwapper = new MaleSwapper(controller, xPos, yPos);
			...
			case "E": {
				int xPos = Integer.parseInt(currentItem[1]);
				int yPos = Integer.parseInt(currentItem[2]);
				FemaleSwapper newFemaleSwapper = new FemaleSwapper(controller, xPos, yPos);
			...
			case "N": {
				...
				StopSign newStopSign = new StopSign(controller, xPos, yPos);
	...
	public static void setControlller(Controller controller) {
		for (Power p : POWER_ARRAY_LIST) {
			p.setController(controller);
		}
		
		for (Rat r : RAT_ARRAY_LIST) {
